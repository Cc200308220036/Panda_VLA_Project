...
class ScriptedPickPlaceExpert:
    ...
    def get_current_target(self):
        ...
        # 关键：放置补偿后的目标 xy
        # 如果 place_err = final_cube_xy - target_xy，
        # 那么 place_xy_offset 应该取 -mean(place_err)
        ...
        # 关键：进入放置阶段时，只初始化一次 target_xy
        # 不要每一步都跟随 target_pos 变化，否则轨迹会抖
        if self.target_xy is None and self.stage in [
            PickPlaceStage.MOVE_ABOVE_TARGET,
            PickPlaceStage.MOVE_TO_PLACE,
            PickPlaceStage.OPEN_GRIPPER,
            PickPlaceStage.RETREAT,
        ]:
            self.target_xy = place_xy.copy()
