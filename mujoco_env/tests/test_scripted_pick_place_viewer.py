...
def main():
    ...
    with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
        while viewer.is_running() and step < max_steps:
            ...
            if expert_info["stage"] == "LIFT" and step % 20 == 0:
                print(
                    "DEBUG_LIFT "
                    f"action={action} "
                    f"ctrl_current_pos={ctrl_info['current_pos']} "
                    f"ctrl_target_pos={ctrl_info['target_pos']} "
                    f"ctrl_delta={ctrl_info['delta']} "
                    f"ctrl_q0_7={ctrl[:7]} "
                    f"joint_pos={env.get_joint_pos().reshape(-1)}"
                )
            ...
            if step % 20 == 0:
                ...
                z_err = abs(pos_err[2])
                gripper_qpos = env.get_gripper_qpos()
                print(
                    f"step={step:04d} "
                    f"stage={expert_info['stage']:20s} "
                    f"ee={ee_pos} "
                    ...
                    f"ik_err={ctrl_info['ik_error']:.4f} "
                    f"grasp={env.is_grasp_success()} "
                    f"place={env.is_place_success(strict=False)}"
                    f"gripper_qpos={gripper_qpos} "
                    f"gripper_ctrl={ctrl_info['gripper_ctrl']} "
