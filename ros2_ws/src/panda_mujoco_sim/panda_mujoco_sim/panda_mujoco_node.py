...
class PandaMujocoNode(Node):
    def __init__(self):
        ...
        # Main ROS timer, 100 Hz
        self.timer = self.create_timer(0.01, self.on_timer)

        # Publish image every N ticks.
        # 10 means 100 Hz / 10 = 10 Hz image.
        self.image_publish_divider = 10

        # Increase substeps for smoother actuator tracking.
        self.n_substeps_per_timer = 10
    ...
    def on_timer(self):
        ...
        # Step MuJoCo physics only. No image rendering here.
        self.env.step_sim(
            ctrl=self.current_ctrl,
            n_substeps=self.n_substeps_per_timer,
        )

        stamp = self.get_clock().now().to_msg()

        # State observation does not include image.
        obs_state = self.env.get_state_obs()

        self.publish_joint_states(obs_state, stamp)
        self.publish_ee_pose(obs_state, stamp)

        # Render and publish image at lower rate.
        if self.tick_count % self.image_publish_divider == 0:
            rgb = self.env.render()
            self.publish_image_rgb(rgb, stamp)
    ...
    def publish_image_rgb(self, rgb, stamp):
        msg = self.bridge.cv2_to_imgmsg(rgb, encoding="rgb8")
        msg.header.stamp = stamp
        msg.header.frame_id = "front_camera"
        self.image_pub.publish(msg)
